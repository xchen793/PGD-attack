...
def main(args):
    ...
    def eval(d,similarity = [],bad = 0,wer_ = 0,fail=0):
        ...
        for idx in range(args.start_index, args.start_index+args.num_samples):
            if d[idx][0]=='success':
                ...
                fail+=1
        return similarity, bad, wer_, fail, avg_sim
        
    similarity, bad, wer_, fail, avg_sim = eval(d)

    print("\n\n")
    print("*********** RESULTS ***********")
    print("\n")

    print(f'Average semantic similarity in total attacks: \t {avg_sim}')
    ...
    print(f'Number of failed attacks due to low cosine similarity: \t {bad}')
